...
def parse_config():
    ...
    parser.add_argument('--model', type=str, default='pv_rcnn', help='model used to predict the boxes')
    ...
    parser.add_argument('--points_path',  type=str, default='D:\\Ian\\UNI\\5_Master_CV\\M9_TFM\\media\\kitti\\training\\velodyne\\000008.bin')
    parser.add_argument('--detection_path', type=str, default='D:\\Ian\\UNI\\5_Master_CV\\M9_TFM\\results\\openpcdet\\0_demo\\000008_pred_pv_rcnn.pkl')
...
def main():
    ...
    points = np.fromfile(args.points_path,
                            dtype=np.float32).reshape(-1, 4)
    
    with open(args.detection_path,
                'rb') as f:
        pred = pickle.load(f)
