...
class KittiDataset(DatasetTemplate):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                def _rot_matrix():
                    ...
                    return np.row_stack((np.column_stack((Rz @ Rx, [0, 0, 0])), [0, 0, 0, 1]))

                def _cart_to_hom(pts):
                ...
                H = _rot_matrix()
                loc_lidar = []
                for point in _cart_to_hom(loc):
                    new_point = H @ point
                    loc_lidar.append(new_point[:3] / new_point[3])

                loc_lidar = np.array(loc_lidar)
                loc_lidar[:, 2] += h[:, 0] / 2
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def generate_single_sample_dict(batch_index, box_dict):
            ...
            pred_boxes_img = np.zeros(shape=(pred_boxes_camera.shape[0], 4))
